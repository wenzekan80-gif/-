...
function publicState(room) {
  return {
    ...
    // ✅ Overtime
    inOvertime: !!room.inOvertime,
    overtimeRemaining: room.overtimeRemaining || 0,
 ...
 * Overtime (清池 + 补池 + 三轮加赛)
 * ------------------------------ */
function drainSupportPoolToLowest(room) {
  while ((room.supportPool || 0) > 0) {
    let minS = Infinity;
    for (const p of room.players) minS = Math.min(minS, p.S);

    const target = room.players.find(p => p.S === minS);
    if (!target) break;

    room.supportPool -= 1;
    const before = target.S;
    target.S = clamp(target.S + 1, 0, 10);
    log(room, `🧹 清池：${target.name}（最低支持）获得 +1支持 ${before}→${target.S}（池剩${room.supportPool}）`);
  }
}

function startOvertime(room) {
  room.inOvertime = true;
  room.overtimeRemaining = OVERTIME_TURNS;

  log(room, `⏱️ 常规回合结束：进入结尾清算流程。`);

  // 1) 清空民意池：全部打给最低支持者直到池=0
  drainSupportPoolToLowest(room);

  // 2) 补上民意池（用初始化公式）
  room.supportPool = 8 + room.players.length;
  log(room, `🫙 补池：supportPool 重新补到 ${room.supportPool}。`);

  // 3) 加赛开始
  log(room, `🔥 进入加赛：还剩 ${room.overtimeRemaining} 轮。`);
}
...
// ✅ 放宽政变条件（W>=2；民粹/独裁门槛降低）
function canLaunchCoup(room, p) {
  const W = p.coupW || 0;
  if (W < 2) return { ok: false, reason: "威胁条不足（需W≥2）" };

  if (p.role === ROLE.POPULIST) {
    if (p.S < 5) return { ok: false, reason: "支持不足（民粹需S≥5）" };
    if (p.T < 1) return { ok: false, reason: "稳定不足（民粹需T≥1）" };
    return { ok: true };
  }
  if (p.role === ROLE.AUTOCRAT) {
    if (p.S < 6) return { ok: false, reason: "支持不足（独裁需S≥6）" };
    if (p.T < 3) return { ok: false, reason: "稳定不足（独裁需T≥3）" };
    return { ok: true };
  }
...
function startGame(room) {
  ...
  // Overtime reset
  room.inOvertime = false;
  room.overtimeRemaining = 0;
...
function advanceRound(room) {
  ...
  const label = room.inOvertime
    ? `🔥 加赛：总统 ${room.players[room.presidentIdx].name}（加赛剩余${room.overtimeRemaining}轮）`
    : `🔁 新回合：总统：${room.players[room.presidentIdx].name}（回合${room.turn}/${MAX_TURNS}）`;
...
function cleanupAndMaybeAdvance(room) {
  ...
  // ✅ 常规回合结束：进入加赛流程（只触发一次）
  if (!room.inOvertime && room.turn >= MAX_TURNS) {
    startOvertime(room);
    advanceRound(room); // 开始加赛第1轮
    return;
  }

  // ✅ 加赛：每回合结束扣 1；扣完后总评
  if (room.inOvertime) {
    room.overtimeRemaining = Math.max(0, (room.overtimeRemaining || 0) - 1);

    if (room.overtimeRemaining <= 0) {
      const sorted = room.players.slice().sort((a, b) => (b.S - a.S) || (b.T - a.T) || (b.M - a.M));
      const w = sorted[0];
      endGame(room, `${w.name}（加赛胜出）`, `加赛结束，按支持/稳定/筹码结算最高者胜。`);
      return;
    }

    log(room, `🔥 加赛回合结束：剩余 ${room.overtimeRemaining} 轮。`);
    advanceRound(room);
    return;
  }
